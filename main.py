...
def main():
    ...
    def insert(img_to_overlay, holiday_insert):
        ...
        img_to_overlay.paste(holiday_insert, (0, 0), mask=holiday_insert)
        img_to_overlay.save(sheet_name, format="png")
            ...
            for single_date in daterange(start_date, end_date):
                ...
                match datetime.strftime(single_date, "%Y-%m-%d"):
                    case "2022-02-14":  # Valentine's Day.
                        insert(img_in_memory, valentines_day.copy())
                    ...
                    case "2022-10-31":  # Halloween.
                        insert(img_in_memory, halloween.copy())
                ...
                match us_holidays.get(f"{single_date}"):
                    case "New Year's Day":
                        closed(img_in_memory)
                        insert(img_in_memory, new_years_day.copy())
                    case "New Year's Day (Observed)":
                        closed(img_in_memory)
                    case "Martin Luther King Jr. Day":
                        closed(img_in_memory)
                        insert(img_in_memory, mlk_day.copy())
                    case "Good Friday":
                        closed(img_in_memory)
                        insert(img_in_memory, good_friday.copy())
                    case "Memorial Day":
                        closed(img_in_memory)
                        insert(img_in_memory, memorial_day.copy())
                    case "Independence Day":
                        closed(img_in_memory)
                        insert(img_in_memory, independence_day.copy())
                    case "Independence Day (Observed)":
                        closed(img_in_memory)
                    case "Labor Day":
                        closed(img_in_memory)
                        insert(img_in_memory, labor_day.copy())
                    case "Veterans Day":
                        closed(img_in_memory)
                        insert(img_in_memory, veterans_day.copy())
                    case "Veterans Day (Observed)":
                        closed(img_in_memory)
                    case "Thanksgiving":
                        closed(img_in_memory)
                        insert(img_in_memory, thanksgiving.copy())
                    case "Christmas Eve":
                        closed(img_in_memory)
                        insert(img_in_memory, christmas_eve.copy())
                    case "Christmas Eve (Observed)":
                        closed(img_in_memory)
                    case "Christmas Day":
                        closed(img_in_memory)
                        insert(img_in_memory, christmas_day.copy())
                    case "New Year's Eve":
                        closed(img_in_memory)
                        insert(img_in_memory, new_years_eve.copy())
